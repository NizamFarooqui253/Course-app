...
const signUp=async(req,res)=>{
  try {
    ...
    const signUpUser=await UserDB.create({...req.body,password:hashPassword})
...
const login=async(req,res)=>{
  try {
    ...
    const token=jwt.sign({id:existUser._id,email:existUser.email},JWT_PASSWORD,{expiresIn:"1d"}); 
